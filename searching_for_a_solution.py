...
numerical_features = ['LotArea', 'YearBuilt', '1stFlrSF', '2ndFlrSF', 'FullBath', 'BedroomAbvGr',
                      ...
                      'GarageCars', 'EnclosedPorch', 'BsmtFinSF1']
categorical_features = ['MSZoning', 'CentralAir', 'KitchenQual', 'Neighborhood', 'Condition1', 'Heating', 'LandContour', 'GarageFinish']
X.fillna({'Electrical': 'SBrkr', 'GarageYrBlt': X['GarageYrBlt'].mean(skipna=True), 'BsmtCond' : 'TA', 'GarageFinish' : 'Unf', 'BsmtFinType1' : 'Unf'}, inplace=True)
...
min_errors_adaboost.append(calculate_min_error_for_adaboost_regressor(train_X, train_y, val_X, val_y,
                                                                      decision_tree_max_depth_range=range(18, 24),
                                                                      n_estimators_range=range(24, 28),
                                                                      print_parameters=True))
